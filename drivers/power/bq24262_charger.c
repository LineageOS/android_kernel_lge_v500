...
static int bq24262_get_prop_batt_status(struct bq24262_chip *chip)
{
	...
	if((chg_type == POWER_SUPPLY_CHARGE_TYPE_TRICKLE || chg_type == POWER_SUPPLY_CHARGE_TYPE_FAST)
			&& ((chg_batt_temp_state == CHG_BATT_STOP_CHARGING_STATE) && (pseudo_ui_charging == 0))
			&& (chip->chg_cable_status == BQ_CHG_STATUS_PRE_CHARGE) && capacity < 100) {
	        batt_status = POWER_SUPPLY_STATUS_NOT_CHARGING;
        	return batt_status;
	}
...
static void bq24262_monitor_batt_temp(struct work_struct *work)
{
	...
	schedule_delayed_work(&chip->battemp_work, msecs_to_jiffies(MONITOR_BATTEMP_POLLING_PERIOD));
}
